...
class Cliente
{
    ...
    public function create($datos)
    {
        ...
        $this->db->query('INSERT INTO clients(client, logo, estado) VALUES (:cliente, :logo, :estado)');
    ...
    public function update($datos, $id)
    {
        ...
        $this->db->query('UPDATE clients SET client = :cliente, logo = :logo, estado = :estado WHERE id = :id');
    
        // Asigna valores a los parámetros de la consulta
        $this->db->bind(':cliente', $datos['client']);
        $this->db->bind(':logo', $datos['logo']);
        $this->db->bind(':estado', $datos['estado']);
        $this->db->bind(':id', $id); // Aquí se utiliza el parámetro :id
